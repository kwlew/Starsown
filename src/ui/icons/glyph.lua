...
--- a unit-box polygon placed at (x, y) and scaled to s. Reuses one scratch
-- buffer, so it allocates nothing per draw.
---@param mode "fill"|"line"
---@param x number
---@param y number
---@param s number # size in screen pixels
---@param unit number[] # flat x, y pairs in 0..1
...
---@param x2 number # 0..1
---@param y2 number # 0..1
---@param radius? number # 0..1 corner radius
...
---@alias GlyphDraw fun(x: number, y: number, s: number) top-left corner, and the box's size in screen pixels
local glyphs = {}

--- a tapered base under a barrel, with a domed turret at its top
...
--- a dot with three arcs radiating from it, like a signal
...
--- an outlined shield with a cross through it
...
--- a ringed circle with a vertical bar, banded top and bottom
...
--- a hub inside a ring, with eight spokes out past it
...
--- a ring and centre dot between four ticks
...
--- a filled teardrop with a pinched waist
function glyphs.flame(x, y, s)
    poly("fill", x, y, s, FLAME)
end

--- three crossed spokes, each tip barbed like a snowflake's
...
--- a cup on a stem and plinth, with a handle each side
...
--- Draws in the current colour, so tinting is the caller's job. Stroke weight
-- scales with size, so every size reads as the same weight. An unknown name
-- falls back to the lock instead of throwing: an unfinished icon should look
-- unfinished, not crash.
---@param name string
---@param x number
---@param y number
