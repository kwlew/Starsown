...
-- use and cached, so an unused mark never pays the flatten-to-mesh cost.
...
function Marks.get(name)
    ...
    if not icon then
        local def = assert(PATHS[name], "unknown mark: " .. tostring(name))
        icon = SvgIcon.new(def.path, def.viewBox)
        cache[name] = icon
    end
    return icon
end
