...
--
-- Succeeds the older lib/onlineCount.lua. Unrelated to lib/presence.lua, which
-- is Discord Rich Presence.
...
local WORKER = "lib/threads/stats.lua"
...
-- makes `require "lib.stats"` work at all is standard Lua's package.path
...
local function newWorkerThread()
    ...
    local path = package.searchpath and package.searchpath("lib.threads.stats", package.path)
...
local function dispatch()
    ...
    -- this way the thread needs no module of ours at all (its `require` can't
    -- see lib/ once the game is packaged into a .love).
    jobChannel:push{
